...
const solutionNode = async (state, config) => {
    ...
    const [mistral_solution, cohere_solution] = await Promise.all([
        mistralModel.invoke(state.messages[0].content, { callbacks, runName: "mistral" }),
        cohereModel.invoke(state.messages[0].content, { callbacks, runName: "cohere" }),
    ]);
    return {
...
const judgeNode = async (state) => {
    ...
    const judgeResponse = await judge.invoke({
        messages: [
            new HumanMessage(`You are the judge evaluating two solutions for the question: "${state.messages[0].content}". 
...
const graph = new StateGraph(State)
    ...
    .addEdge("judge", END)
    .compile();
export async function streamGraph(userMessage) {
